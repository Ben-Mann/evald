 ...
 */
public abstract class AbstractEvald {

    private static final Pattern validTokenPattern = Pattern.compile("^[a-zA-Z_][a-zA-Z_0-9]*$");
    protected static final int ARRAY_RESIZE_BUFFER = 16;
    ...
    final Set<String> undeclaredKeyMap = new HashSet<>();
    final List<SetValueArrayCallback> valueArrayCallbacks = new ArrayList<>();
    final Set<Integer> usedIndices = new HashSet<>();

    ParserList<ValueParser> valueParsers = new ParserList<>();
    ParserList<BinaryOperatorParser> binaryOperatorParsers = new ParserList<>();
    ParserList<PrefixOperatorParser> prefixOperatorParsers = new ParserList<>();
    ParserList<PostfixOperatorParser> postfixOperatorParsers = new ParserList<>();

    /**
     * Construct with only basic arithmetic support ({@link LibArithmetic Library.CORE})
     */
    public AbstractEvald() {
     ...
     */
    public AbstractEvald(Library... libraries) {
     ...
     * implementation; if a library function has been overridden with {@link #addUserFunction(ArgFunction)}
    ...
    /**
     * Reset the variables from the last parsed expression.
     */
    public void reset() {
     ...
     */
    protected Node parseNode(String expression) {
        ...
        Node root = new ExpressionParser(this, string).parse();
     ...
     * 
     * @param enabled
     *            true to enable implicit multiplication (the default), false to disable it.
     */
    ...
    public void removeConstant(String token) {
        ...
        List<ValueParser> toRemove = new ArrayList<>();
