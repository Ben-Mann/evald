...
public class Evald {
    ...
    final Pattern subExpressionPattern = Pattern.compile("^\\s*([a-zA-Z_][a-z_A-Z0-9]*)\\s*=([^=]+.*)");
    ...
    private void parseSubExpressions(String expression) {
        ...
        if (statements.length == 1) {
            if (!subExpressionPattern.matcher(statements[0]).find()) {
                parseSubExpression(resultVariable + " = " + statements[0]);
                return;
            }
        }
        for (String statement : statements) {
            ...
            parseSubExpression(statement);
        }
    }

    private void parseSubExpression(String subExpression) {
